...
story += [p('4. Verifikasi dan sumber', 'H1x'), p('Verifikasi lokal yang dilakukan:', 'Bodyx'), table([[p('Pemeriksaan', 'CellHead'), p('Hasil', 'CellHead')], [p('php artisan migrate:fresh --seed', 'Cell'), p('Berhasil membuat tabel domain dan seed 12 saham tanpa membuat kuotasi dummy.')], [p('php artisan market:tick', 'Cell'), p('Berhasil memperbarui kuotasi live dan membuat batch broadcast.')], [p('Live Zapi tick', 'Cell'), p('Request bulk berhasil; snapshot menghasilkan source=live dan 12 quote IDX.')], [p('ZapiMarketDataTest', 'Cell'), p('Berhasil menormalisasi close, previous, volume, dan API key menggunakan Http::fake.')], [p('php artisan test --compact', 'Cell'), p('7 tests passed, 22 assertions.')], [p('npm run build', 'Cell'), p('Berhasil menghasilkan manifest dan aset production Vite.')], [p('vendor/bin/pint --dirty --format agent', 'Cell'), p('Berhasil memformat file PHP yang diubah.')]], [65*mm, 95*mm]), Spacer(1, 8), p('Sumber teknis:', 'H2x'), p('1. IDX Data Services: https://www.idx.id/en/products/idx-data-services/ (lisensi direct/redistributor, produk real-time, delayed, dan EoD).', 'Smallx'), p('2. Zapi IDX API: https://zpi.web.id/api/finance/idx (stock-summary dan stock-history; API key server-side).', 'Smallx'), p('3. Laravel HTTP Client: https://laravel.com/docs/13.x/http-client (timeout, connectTimeout, retry, dan error handling).', 'Smallx'), p('4. Laravel Broadcasting, Reverb, dan Scheduling: https://laravel.com/docs/13.x/broadcasting, https://laravel.com/docs/13.x/reverb, https://laravel.com/docs/13.x/scheduling', 'Smallx'), p('5. Laravel Deployment dan Docker Compose: https://laravel.com/docs/13.x/deployment dan https://docs.docker.com/compose/how-tos/production/', 'Smallx'), Spacer(1, 10), p('Kesimpulan desain: adapter provider menjaga kontrak dashboard tetap stabil, live quote tersedia saat key dan lisensi siap, dan kegagalan provider mempertahankan data live terakhir tanpa membuat data pengganti.', 'Bodyx')]
