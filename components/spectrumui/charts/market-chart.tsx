...
export function MarketChart({
    ...
    const priceFormatter =
        formatPrice ?? ((value: number) => formatMoney(value, compactPrice));
    const reduce = usePrefersReducedMotion();
    const [wrapRef, width] = useElementWidth<HTMLDivElement>();
    const [rangeLabel, setRangeLabel] = React.useState(
        () =>
            ranges.find((r) => r.label === defaultRange)?.label ??
            ranges[ranges.length - 1].label,
    );
    ...
    React.useEffect(() => {
        ...
        const id = window.setInterval(() => {
            ...
            const next: Candle = {
                ...
                open: last.close,
                high: round2(
                    Math.max(last.close, close) * (1 + rand() * 0.006),
                ),
    ...
    const series = React.useMemo(
        () => (ticks.length ? [...data, ...ticks] : data),
        [data, ticks],
    );

    const range =
        ranges.find((r) => r.label === rangeLabel) ?? ranges[ranges.length - 1];
    ...
    const volH = showVolume
        ? Math.round((innerBottom - innerTop) * VOLUME_SHARE)
        : 0;
    ...
    const delta = baseline
        ? (((shown?.close ?? 0) - baseline) / baseline) * 100
        : 0;
    const up = delta >= 0;
    const dirColor = up
    ...
    const timeTicks = React.useMemo(() => {
        ...
        const minGap = 62;
        const xOf = (i: number) =>
            plotX0 + (plotW / Math.max(n, 1)) * (i + 0.5);
        if (out[out.length - 1] !== n - 1) {
            while (out.length && xOf(n - 1) - xOf(out[out.length - 1]) < minGap)
                out.pop();
            out.push(n - 1);
        }
        return out;
    ...
    const areaPoints = React.useMemo(() => {
        const values =
            morphed.length === MORPH_SAMPLES ? morphed : resample(closes);
        ...
        >
            ...
            <div className="mb-3 flex flex-wrap items-start justify-between gap-x-4 gap-y-3">
                <div className="min-w-0">
                    <div className="flex items-baseline gap-2">
                        <span className="font-mono text-[12px] font-semibold tracking-wide text-neutral-950 dark:text-white">
                            {symbol}
                        </span>
                            ...
                            <span className="inline-flex items-center gap-1 rounded-full bg-black/[0.05] px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-widest text-neutral-500 dark:bg-white/[0.08] dark:text-neutral-400">
                                <span
                                    ...
                                    style={{
                                        ...
                                        animation: reduce
                                            ? undefined
                                            : "spectrum-mc-fade 1.1s ease-in-out infinite alternate",
                                    }}
                                />
                                Live
                            </span>
                    ...
                    <div className="mt-1 flex items-end gap-2.5">
                        <RollingNumber
                            ...
                            animate={!reduce}
                            className={cn(
                                "font-mono text-[26px] font-semibold text-neutral-950 transition-colors duration-300 dark:text-white",
                            )}
                        />
                        <span
                            className="mb-1 rounded-md px-1.5 py-0.5 font-mono text-[12px] font-semibold tabular-nums transition-colors duration-300"
                            style={{
                                color: dirColor,
                                background: flash
            ...
            <ChartState
                ...
                empty={{
                    title: "No price data",
                    description:
                        "Point the chart at a candle feed and it will render as soon as bars arrive.",
                }}
                onRetry={onRetry}
            >
                <div className="relative w-full" style={{ height }}>
                    {!ready ? null : (
                        <svg
                            ...
                            role="img"
                            aria-label={`${symbol} ${name} price chart, ${rangeLabel} range. ${priceFormatter(
                                last?.close ?? 0,
                            )}, ${formatSignedPct(delta)}.`}
                            tabIndex={0}
                            onKeyDown={onKeyDown}
                            onPointerMove={(e) =>
                                pointerToIndex(e.clientX, e.clientY)
                            }
                            onPointerDown={(e) =>
                                pointerToIndex(e.clientX, e.clientY)
                            }
                        ...
                        >
                            <defs>
                                ...
                                >
                                    <stop
                                        offset="0%"
                                        stopColor="var(--spectrum-chart-up)"
                                        stopOpacity={0.28}
                                    />
                                    <stop
                                        offset="100%"
                                ...
                                >
                                    <stop
                                        offset="0%"
                                        stopColor="var(--spectrum-chart-down)"
                                        stopOpacity={0}
                                    />
                                    <stop
                                        offset="100%"
                            ...
                            <g className="font-mono">
                                {priceTicks.map((tick) => {
                                    ...
                                    const collides =
                                        (last != null &&
                                            Math.abs(y - priceY(last.close)) <
                                                13) ||
                                        (hoverY != null &&
                                            Math.abs(y - hoverY) < 13);
                                    ...
                                    <path
                                        ...
                                        style={
                                            reduce
                                                ...
                                                : {
                                                      animation:
                                                          "spectrum-mc-fade 620ms ease-out both",
                                                  }
                                        }
                                    />
                                    <path
                                        ...
                                        style={
                                            reduce
                                                ...
                                                : {
                                                      strokeDasharray: 1,
                                                      animation:
                                                          "spectrum-mc-draw 900ms cubic-bezier(0.22, 1, 0.36, 1) both",
                                                  }
                                        }
                                    />
                                    <path
                                    ...
                                    {live && !reduce ? (
                                        <>
                                            <circle
                                                ...
                                                style={{
                                                    animation:
                                                        "spectrum-mc-ping 1.6s ease-out infinite",
                                                }}
                                            />
                                            <circle
                                                cx={cx(n - 1)}
                            ...
                            {hoverIndex != null && view[hoverIndex] ? (
                                ...
                                >
                                    ...
                                    <circle
                                        ...
                                        fill="var(--spectrum-chart-surface)"
                                        stroke={
                                            view[hoverIndex].close >=
                                            view[hoverIndex].open
                                                ? "var(--spectrum-chart-up)"
                                                : "var(--spectrum-chart-down)"
                                        }
                                        strokeWidth={2}
