...
export function generateCandles({
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        const volume = Math.round(
            (0.55 + rand() * 0.7 + range * 26) * 1_000_000,
        );
...
export function useTween(
    ...
    React.useEffect(() => {
        ...
        const changed =
            to.length !== target.length || target.some((v, i) => v !== to[i]);
        if (!changed) return;
...
export function RollingNumber({
    ...
    return (
        <span
            className={cn(
                "inline-flex items-end leading-none tabular-nums",
                className,
            )}
        >
            ...
            {text.split("").map((char, index) => (
                <Digit
                    key={`${index}-${char >= "0" && char <= "9" ? "digit" : char}`}
                    char={char}
                    animate={animate}
                />
            ))}
...
export function RangeSelector({
    ...
    const index = Math.max(
        0,
        ranges.findIndex((r) => r.label === value),
    );
...
export type TreemapTile = TreemapInput & {
    x: number;
    y: number;
    w: number;
    h: number;
...
export function squarify(
    ...
    if (!sorted.length || totalWeight <= 0 || width <= 0 || height <= 0)
        return [];
        ...
        return (
            <div className="flex h-full items-end gap-2 pb-5 pt-3">
                {SKELETON_BARS.map((h, i) => (
                    <div
                        key={i}
                        className={cn("flex-1", block)}
                        style={{ height: `${h}%`, ...breathe(i) }}
                    />
                ))}
            </div>
        );
        ...
        return (
            <div className="relative flex h-full flex-col justify-between py-4">
                {[0, 1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className="h-px w-full bg-black/[0.05] dark:bg-white/[0.06]"
                    />
                ))}
                <svg
        ...
        return (
            <div className="flex h-full flex-col justify-evenly py-2">
                {SKELETON_ROWS.map((w, i) => (
                    <div
                        key={i}
                        className={cn("h-4", block)}
                        style={{ width: `${w}%`, ...breathe(i) }}
                    />
                ))}
            </div>
        );
        ...
        >
            ...
            <div
                ...
                style={
                    reduce
                        ? undefined
                        : { animation: `spectrum-mc-enter 480ms ${EASE} both` }
                }
            >
                <span
                    className={cn(
...
export function ChartState({
    ...
    if (status === "empty")
        return <ChartEmpty height={height} variant={variant} {...empty} />;
    if (status === "error") {
        return (
            <ChartError
                height={height}
